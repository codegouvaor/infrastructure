...
display_header() {
    echo ""
    echo "Infrastructure Gouv Website container"
    echo ""
...
run_server() {
    ...
    if [ "${NODE_ENV}" = "production" ]; then
        log_info "Infrastructure Gouv Website frontend starting (static)"
    ...
    log_info "Infrastructure Gouv Website frontend starting (development)"
    log_info "Frontend listening on 0.0.0.0:${FRONTEND_PORT}"
...
run_worker() {
    ...
    log_info "Infrastructure Gouv Website API starting"
...
run_air() {
    ...
    log_info "Infrastructure Gouv Website API starting (hot-reload)"
